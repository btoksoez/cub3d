...
/*   Updated: 2024/06/12 18:45:17 by andre-da         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_player *player)
{
	...
	{
		max_dist_y = (MINI_ROWS * MINI_SCALE) / 2 + fabs(((HEIGHT - (HEIGHT / 5)
						+ (HEIGHT / SCALE)) + ((MINI_SCALE * 5) / 2)
					- MINI_PCENTER) - ((HEIGHT - (HEIGHT / 5) + (HEIGHT
							/ SCALE)) + (player->pos.y / 2)));
		max_dist_x = (MINI_COLS * MINI_SCALE) / 2 + fabs(((WIDTH - (WIDTH / 5)
						+ (WIDTH / SCALE)) + ((MINI_SCALE * 7) / 2)
					- MINI_PCENTER) - ((WIDTH - (WIDTH / 5) + (WIDTH / SCALE))
					+ (player->pos.x / 2)));
	}
	else if (player->pos.y <= mini.vert_vision)
	{
		max_dist_y = ((MINI_ROWS * MINI_SCALE) / 2) + fabs(((HEIGHT - (HEIGHT
		...
		t_minimap mini)
{
	if ((player->pos.x <= mini.hori_vision)
		&& (player->pos.y <= mini.vert_vision))
	{
		draw_line(game, (((WIDTH - (WIDTH / 5) + (WIDTH / SCALE))
					+ (player->pos.x / 2)) + MINI_PCENTER), (((HEIGHT - (HEIGHT
							/ 5) + (HEIGHT / SCALE)) + (player->pos.y / 2))
				+ MINI_PCENTER), (((WIDTH - (WIDTH / 5) + (WIDTH / SCALE))
					+ (player->pos.x / 2)) + MINI_PCENTER) + (ray->dir.x
				* ray->len), (((HEIGHT - (HEIGHT / 5) + (HEIGHT / SCALE))
					+ (player->pos.y / 2)) + MINI_PCENTER) + (ray->dir.y
				* ray->len), BLUE);
	}
	else if (player->pos.y <= mini.vert_vision)
	{
		draw_line(game, (((WIDTH - (WIDTH / 5) + (WIDTH / SCALE)) + ((MINI_SCALE
							* 7) / 2) - MINI_PCENTER) + MINI_PCENTER), (((HEIGHT
						- (HEIGHT / 5) + (HEIGHT / SCALE)) + (player->pos.y
						/ 2)) + MINI_PCENTER), (((WIDTH - (WIDTH / 5) + (WIDTH
							/ SCALE)) + ((MINI_SCALE * 7) / 2) - MINI_PCENTER)
				+ MINI_PCENTER) + (ray->dir.x * ray->len), (((HEIGHT - (HEIGHT
							/ 5) + (HEIGHT / SCALE)) + (player->pos.y / 2))
				+ MINI_PCENTER) + (ray->dir.y * ray->len), BLUE);
	}
	else if (player->pos.x <= mini.hori_vision)
	{
		draw_line(game, (((WIDTH - (WIDTH / 5) + (WIDTH / SCALE))
					+ (player->pos.x / 2)) + MINI_PCENTER), (((HEIGHT - (HEIGHT
							/ 5) + (HEIGHT / SCALE)) + ((MINI_SCALE * 5) / 2)
					- MINI_PCENTER) + MINI_PCENTER), (((WIDTH - (WIDTH / 5)
						+ (WIDTH / SCALE)) + (player->pos.x / 2))
				+ MINI_PCENTER) + (ray->dir.x * ray->len), (((HEIGHT - (HEIGHT
							/ 5) + (HEIGHT / SCALE)) + ((MINI_SCALE * 5) / 2)
					- MINI_PCENTER) + MINI_PCENTER) + (ray->dir.y * ray->len),
			BLUE);
	}
	else
	{
		draw_line(game, (((WIDTH - (WIDTH / 5) + (WIDTH / SCALE)) + ((MINI_SCALE
							* 7) / 2) - MINI_PCENTER) + MINI_PCENTER), (((HEIGHT
						- (HEIGHT / 5) + (HEIGHT / SCALE)) + ((MINI_SCALE * 5)
						/ 2) - MINI_PCENTER) + MINI_PCENTER), (((WIDTH - (WIDTH
							/ 5) + (WIDTH / SCALE)) + ((MINI_SCALE * 7) / 2)
					- MINI_PCENTER) + MINI_PCENTER) + (ray->dir.x * ray->len),
			(((HEIGHT - (HEIGHT / 5) + (HEIGHT / SCALE)) + ((MINI_SCALE * 5)
						/ 2) - MINI_PCENTER) + MINI_PCENTER) + (ray->dir.y
				* ray->len), BLUE);
	}
}
